-- ============================================================
-- Migration 022 - Atualiza seed da demo
-- Substitui "Jon Snow" (ficção óbvia) por "Carlos Mendes"
-- (nome fictício realista, sem referência cultural explícita)
-- Estrutura e volume idênticos ao migration-021.
-- ============================================================
...
begin
    ...
    -- ── 5 variações de currículo do mesmo candidato (Carlos Mendes) ──
    insert into demo_cv_versions (session_id, name, description, file_name, active, created_at) values
        (p_session_id, 'QA Sênior — Padrão',         'Versão geral para candidaturas sênior',                 'cv-carlos-mendes-qa-senior.pdf',    true,  now() - interval '35 days')
        returning id into cv1;
    insert into demo_cv_versions (session_id, name, description, file_name, active, created_at) values
        (p_session_id, 'QA Automation — Remoto',     'Foco em Playwright/Selenium, vagas remotas',            'cv-carlos-mendes-qa-auto.pdf',      true,  now() - interval '12 days')
        returning id into cv2;
    insert into demo_cv_versions (session_id, name, description, file_name, active, created_at) values
        (p_session_id, 'SDET — Mercado Financeiro', 'Adaptado para fintechs',                                 'cv-carlos-mendes-sdet.pdf',         true,  now() - interval '10 days')
        returning id into cv3;
    insert into demo_cv_versions (session_id, name, description, file_name, active, created_at) values
        (p_session_id, 'Test Analyst — Pleno',       'Versão para vagas pleno',                               'cv-carlos-mendes-test-analyst.pdf', false, now() - interval '180 days')
        returning id into cv4;
    insert into demo_cv_versions (session_id, name, description, file_name, active, created_at) values
        (p_session_id, 'QA Bilíngue — EN',           'Versão bilíngue para empresas internacionais',          'cv-carlos-mendes-bilingue.pdf',     true,  now() - interval '55 days')
